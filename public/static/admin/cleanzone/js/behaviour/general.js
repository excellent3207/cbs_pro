...
$(function(){
	...
	$('body').append('<div class="md-modal colored-header success md-effect-10" id="mod-table-select"><div class="md-content">' +
			      ...
			      '<div class="modal-footer clearFix">' +
			        ...
			        '<button type="button" class="btn btn-primary btn-flat md-process">确定</button>' +
			      '</div>' +
			    '</div>' +
			'</div>' +
...
function syhInfo(title, content, callback){
	...
	$('#mod-table-select .md-process')[0].onclick = function(){
		if(this.callback){
			if(this.callback($('#mod-table-select'))){
				$('#mod-table-select').niftyModal('hide');
			}
		}else{
			$('#mod-table-select').niftyModal('hide');
		}
...
function syhInfoError(msg){
	$('#mod-table-select .modal-footer .btn-syh-cancel-wrap').html('<span style="color:red">'+msg+'</span>');
}
function syhTableHidden(){
	$('#mod-table-select').niftyModal('hide');
}
...
function updateSelectModalTableRadio(data, fields, pagination, pageClickCallback){
	updateSelectModalTable(data, pagination);
}
...
function createSelected(){
	...
	for(var id in tableSelected){
		...
		dArr.reverse();
		var name = tableSelected[id]['name'] ?  tableSelected[id]['name'] : id;
		if(name.length>20) name = name.substring(0, 10) + '...';
		selectHtml += '<a data-popover="popover" data-original-title="详细信息" data-content="'+dArr.join(',')+
			...
			'" class="btn btn-info btn-syh-cancel">'+name+'<i>&times;</i></a>'
	}
...
function updateSelectTag(selected, tagWrap, inputObj){
	...
	for(var k in selected){
		...
		dArr.reverse();
		var name = selected[k]['name'] ?  selected[k]['name'] : k;
		if(name.length>20) name = name.substring(0, 10) + '...';
	...
	inputObj.val(ids.join(','));
	if(ids.length>0){
		parsleyRemoveError(inputObj);
	}
}
function formatSelected(selected, showField){
	for(var k in selected){
		if(!selected[k]['name']){
			selected[k]['name'] = selected[k][showField];
			delete selected[k][showField];
		}
	}
	return selected;
}
...
function parsleyRemoveError(obj){
	var pid = obj.data('parsley-id');
