...
class TnCode{
    ...
    function check($offset=''){
        ...
        $ret = abs(session('tncode_r')-$offset)<=$this->_fault;
        if($ret){
            session('tncode_r', null);
        }else{
            $err = session('tncode_err');
            $err++;
            session('tncode_err', $err);
            if($err>10){//错误10次必须刷新
                session('tncode_r', null);
            }
        }
    ...
    private function _init(){
        ...
        $file_bg = App::getRootPath().'public/static/admin/tncode/bg/'.$bg.'.png';
        ...
        session('tncode_r', $this->_x);
        session('tncode_err', 0);
    ...
    private function _imgout(){
        if((!isset($_GET['nowebp']) || !$_GET['nowebp'])&&function_exists('imagewebp')){//优先webp格式，超高压缩率
            $type = 'webp';
    ...
    private function _createBg(){
        $file_mark = App::getRootPath().'public/static/admin/tncode/img/mark.png';
    ...
    private function _createSlide(){
        $file_mark = App::getRootPath().'public/static/admin/tncode/img/mark2.png';
